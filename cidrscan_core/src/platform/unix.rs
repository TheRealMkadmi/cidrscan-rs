use errno::errno;
use libc::{pthread_mutex_t, pthread_mutexattr_init, pthread_mutexattr_t};
#[cfg(target_os = "linux")]
...
pub fn robust_mutex(
    mutex_ptr: *mut u8,
) -> Result<(Box<dyn raw_sync::locks::LockImpl>, *mut pthread_mutex_t), i32> {
    ...
    let (m_raw, _) =
        unsafe { RawMutex::new(mutex_ptr, &mut attr as *mut _ as _).map_err(|_| errno().0)? };
    Ok((m_raw, mutex_addr))
}
...
pub fn robust_mutex(
    ...
    let mutex_addr = mutex_ptr as *mut pthread_mutex_t;
    unsafe {
        let (m_raw, _) = RawMutex::new(mutex_ptr, ptr::null_mut()).map_err(|_| errno().0)?;
        Ok((m_raw, mutex_addr))
    }
}
